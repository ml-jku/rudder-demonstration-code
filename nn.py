...
class LSTMCell(jit.ScriptModule):
    def __init__(self, n_fwd_features, n_lstm, n_rec_features=None,
        ...
        self.w_fwd_cat = nn.Parameter(torch.zeros((n_fwd_features, n_lstm * n_active_fwds), dtype=torch.float32))
        ...
        self.b_cat = nn.Parameter(torch.zeros((n_lstm * n_active_b,), dtype=torch.float32))
        n_active_b_tickers = len([1 for i in self.lstm_inlets if self.b_tickers_init[i] is not False])
        self.b_tickers_cat = nn.Parameter(torch.zeros((n_lstm * n_active_b_tickers,), dtype=torch.float32))
        ...
        dummy_fwd_inputs = torch.empty(size=(1, self.n_fwd_features), dtype=torch.float32)
        dummy_rec_input = torch.empty(size=(1, self.n_rec_features), dtype=torch.float32)
        dummy_c = torch.empty(size=(1, self.n_lstm), dtype=torch.float32)
    ...
    def __cell_python__(self, fwd_input, rec_input, c_old, w_fwd_cat, w_rec_cat, b):
        ...
        return (c, h, *[v if v is not False else torch.zeros((1,), dtype=torch.float) for v in acts.values()])
...
class LSTMLayer(jit.ScriptModule):
    def __init__(self, in_features, out_features,
                 ...
                 b_ci_tickers=nn.init.normal_, b_ig_tickers=nn.init.normal_, b_og_tickers=nn.init.normal_,
                 b_fg_tickers=False, n_tickersteps=0, inputformat='NLC'):
        ...
        self.lstm_cell = LSTMCell(n_fwd_features=in_features, n_lstm=out_features,
                                  ...
                                  b_fg_tickers=b_fg_tickers)

        # Get initializer for tensors
        self.c_init = c_init
        self.h_init = h_init
        self.c_first = nn.Parameter(torch.zeros((out_features,), dtype=torch.float))
        self.h_first = nn.Parameter(torch.zeros((out_features,), dtype=torch.float))
    ...
    # @torch.jit.script_method  # might lead to imprecise gradients
